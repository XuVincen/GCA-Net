...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, root_dir, split='train', img_size=256, is_train=True,
                 to_tensor=True, use_augment=True):          # 新增 use_augment
...
class CDDataset(ImageDataset):
    ...
    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,
                 to_tensor=True, use_normalize=True, use_augment=True):   # 新增两个参数
        super(CDDataset, self).__init__(root_dir, img_size=img_size, split=split,
                                        is_train=is_train, to_tensor=to_tensor,
                                        use_augment=use_augment)          # 传给父类
        self.label_transform = label_transform
        self.use_normalize = use_normalize                                # 保存
    ...
    def __getitem__(self, index):
        ...
        [img, img_B], [label] = self.augm.transform([img, img_B], [label],
                                                    to_tensor=self.to_tensor,
                                                    use_normalize=self.use_normalize)   # 传入标准化控制
        # print(label.max())
