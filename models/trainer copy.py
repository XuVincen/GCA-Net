...
# from models.losses import cross_entropy, FocalLoss,BoundaryLoss,Boundary_ce_loss,Focal_Dice_BL,FocalLoss_with_dice,FocalDiceLoss,sigmoid_focal_loss
# # 我添加的行下方
...
#         self._pxl_loss = FocalDiceLoss
#         self._pxl_loss1 = sigmoid_focal_loss
#         self._pxl_loss2 = sigmoid_focal_loss
...
#         # print("G_pred shape:", self.G_pred.shape)  # 模型输出的形状（应是4维）
#         gt = self.batch['L'].to(self.device).long()
#         # print("gt shape:", gt.shape)  # 标签的形状（当前是6维，需要确认具体维度）
#         epoch_id = self.epoch_id
...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        # define the loss functions
        #下面两行是我自己的
        self.lambda_loss = getattr(args, 'lambda_loss', 0.75)   # 从 args 中读取 λ 值
        self._pxl_loss = lambda inputs, targets: BCEDiceLoss(inputs, targets, lambda_loss=self.lambda_loss)
        
        #这行是SMOWnet的损失
        # self._pxl_loss = BCEDICE_loss()


        self._pxl_loss1 = cross_entropy
        self._pxl_loss2 = cross_entropy#损失lamada消融专用这四行换上面三行
