...
class CDEvaluator():
    ...
    def _load_checkpoint(self, checkpoint_name='best_ckpt.pt'):
        if os.path.exists(os.path.join(self.checkpoint_dir, checkpoint_name)):
            ...
            try:
                ...
                # 兼容新旧版本state_dict键名
                model_state_key = 'model_G_state_dict' if 'model_G_state_dict' in checkpoint else 'model'
                self.net_G.load_state_dict(checkpoint[model_state_key])
    ...
    def _collect_running_batch_states(self,args):
        ...
        if np.mod(self.batch_id, 100) == 1:
            # ========== 修改开始 ==========
            # 统一使用 nrow=args.batch_size，并对标签添加通道维度
