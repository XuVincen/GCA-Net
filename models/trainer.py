...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        if args.optimizer == "sgd":
            # 为 DVM_Net 使用 1e-5 权重衰减（原始论文值），其他模型使用 5e-4
            wd = 1e-5 if args.net_G == 'DVM_Net' else 5e-4
        ...
        # 核心修改：根据模型选择损失函数
        # =====================================================
        if args.net_G == 'DVM_Net':
            # DVM-Net 原始使用纯 BCEWithLogitsLoss
            self._pxl_loss = torch.nn.BCEWithLogitsLoss()
            self.logger.write('DVM_Net 使用 BCEWithLogitsLoss\n')
        else:
            # 其他模型继续使用您的 BCEDiceLoss
            self.lambda_loss = getattr(args, 'lambda_loss', 0.75)
            self._pxl_loss = lambda inputs, targets: BCEDiceLoss(inputs, targets, lambda_loss=self.lambda_loss)
            self.logger.write('其他模型使用 BCEDiceLoss\n')
        # =====================================================

        # 下面两行是您原来的损失定义（已注释，保留备查）
    ...
    def _visualize_pred(self, args):
        ...
        if args.deep_supervision == True:
            pred = torch.sigmoid(self.G_pred[-1]) > 0.5
        else:
            pred = torch.sigmoid(self.G_pred) > 0.5
        pred_vis = pred.float() * 255  # shape: (B, 1, H, W)
        return pred_vis
    ...
    def _update_metric(self, args):
        ...
        update metric
        """
        target = self.batch['L'].to(self.device).detach()  # (B, H, W)
        if args.deep_supervision == True:
            logits = self.G_pred[-1].detach()
        else:
            logits = self.G_pred.detach()
        # ========== 修改：sigmoid + 阈值，然后 squeeze 掉通道维 ==========
        pred = (torch.sigmoid(logits) > 0.5).long().squeeze(1)  # (B, H, W)
    ...
    def _collect_running_batch_states(self,args):
        ...
        if args.loss_SD == True:
            ...
            if args.deep_supervision == True:
                ...
                # ========== 已经正确使用 squeeze ==========
                self.G_loss = self._pxl_loss(self.G_pred.squeeze(1), gt.float())
        ...
        if np.mod(self.batch_id, 500) == 1:
            vis_input = utils_.make_numpy_grid(de_norm(self.batch['A']), nrow=args.batch_size)
            vis_input2 = utils_.make_numpy_grid(de_norm(self.batch['B']), nrow=args.batch_size)
            vis_pred = utils_.make_numpy_grid(self._visualize_pred(args), nrow=args.batch_size)
            vis_gt = utils_.make_numpy_grid(self.batch['L'].unsqueeze(1), nrow=args.batch_size)
