...
def get_loaders(args):
    ...
    dataConfig = data_config.DataConfig().get_data_config(data_name)
    root_dir = dataConfig.root_dir
    label_transform = dataConfig.label_transform   # 原始默认值（可能为 None）
    split = args.split
    split_val = 'val'
    ...
    if args.net_G == 'DVM_Net':
        # 标签归一化：将 0/255 转为 0/1
        label_transform = 'norm'
        # 图像预处理：不做 ImageNet 标准化
        use_normalize = False
        # 训练时不做数据增强（翻转、裁剪等）
        use_augment = False
    else:
...
def make_numpy_grid(tensor_data, pad_value=0, padding=0, nrow=8):
    tensor_data = tensor_data.detach()
    vis = utils.make_grid(tensor_data, nrow=nrow, pad_value=pad_value, padding=padding)
