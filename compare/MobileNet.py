...
class MobileNetV2(nn.Module):
    def __init__(self, pretrained=None, num_classes=1000, width_mult=1.0):
        ...
        # 添加额外的卷积层来调整通道数，使其与 ResNet18 的输出通道数匹配
        self.adjust_layers = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(24, 64, kernel_size=1),  # 调整第一层特征图的通道数
                nn.BatchNorm2d(64),
                nn.ReLU6(inplace=True)
            ),
            nn.Sequential(
                nn.Conv2d(32, 128, kernel_size=1),  # 调整第二层特征图的通道数
                nn.BatchNorm2d(128),
                nn.ReLU6(inplace=True)
            ),
            nn.Sequential(
                nn.Conv2d(96, 256, kernel_size=1),  # 调整第三层特征图的通道数
                nn.BatchNorm2d(256),
                nn.ReLU6(inplace=True)
            ),
            nn.Sequential(
                nn.Conv2d(320, 512, kernel_size=1),  # 调整第四层特征图的通道数
                nn.BatchNorm2d(512),
                nn.ReLU6(inplace=True)
            )
        ])
    ...
    def forward(self, x):
        ...
        # 选择需要的4个特征图并调整通道数
        selected_features = [res[1], res[2], res[3], res[4]]  # 跳过第一个特征图
        adjusted_features = []
        for i, feat in enumerate(selected_features):
            adjusted_features.append(self.adjust_layers[i](feat))
